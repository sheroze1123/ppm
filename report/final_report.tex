...
\title{Final Project Report\\~\\
	\large Parallel Particle-Mesh Methods Applied to the N-body Problem}
...
Chapter~\ref{chap:sim} discusses a specific application of the n-body simulator and the results of advancing a system of a large number of particles with time. This chapter introduces the correspondence between the simulation scales and the physical system and contains plots of the system evolution.
...
In class, we explored the three body problem, a specific case of the n-body particle-particle model. At each time step of the system, the velocity, position and acceleration attributes of an element need to be updated according to the following set of equations.
\begin{align}
...
To visualize the N-body simulation we marshal particle position data at each time step (implemented in \texttt{marshaller.cpp}) and use MoviePy \cite{moviepy} to create an MP4 representation of the simulation. Further information can be found in \url{https://github.com/sheroze1123/ppm} in \texttt{./scripts/mp4.py}
...
\bibitem{git}
S. Sheriffdeen, M. Whittaker, (n.d.). \textit{Parallel Particle Mesh applied to N-body simulations}. Retrieved December 14, 2015, from https://github.com/sheroze1123/ppm
...
\textit{Solving the collisionless Boltzmann equation using N-body simulations}. (n.d.). Retrieved December 14, 2015, from http://www.mi.infn.it/isapp04/Moore/notes2.pdf
